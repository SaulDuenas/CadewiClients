...
public class async_data extends AsyncTask<Void, Integer, Void>  {
    ...
    Integer DELAY_MINUTES = 5;   // SADB: variable set
    ...
    */
    ...
    private void upload_logger (final Integer id, final String userMail,final String type,final String category,final String device,final String pMessage) {
        ...
        uploadLogger.enqueue(new Callback<LoggerResponse>() {
            ...
            public void onResponse(Call<LoggerResponse> call, Response<LoggerResponse> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if(logresponse.getEstado().equals(Utils.Status_OK)) {
                        ...
                        dbLog.updateLogger(id, userMail, type, category, device, pMessage, Utils.ESTATUS.SEND.toString(),new Date());

                    }
                    else {

                        logger.error(Utils.appUser.correo,"Error from server: ",logresponse.getExcepcion(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                        // se actualiza el estado del logger
                        dbLog.updateLogger(id, userMail, type, category, device, pMessage, Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                    }
                }
                else{
                    ...
                    logger.error(Utils.appUser.correo,"Error HTTP on upload_logger: ",message.toString(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                    dbLog.updateLogger(id, userMail, type, category, device, message.toString(), Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                }
            ...
            public void onFailure(Call<LoggerResponse> call, Throwable t) {
                ...
                dbLog.updateLogger(id, userMail, type, category, device, pMessage, Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                logger.error(Utils.appUser.correo,"onFailure uploadLogger.enqueue: ",message.toString(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                // SADB:  Enabled Process
                Upload_Data = false;
            }
        });
    ...
    private void upload_Photo(final Integer photoId ,Integer client,Integer address,String pathfile,String note)  {
        ...
        uploadPhoto.enqueue(new Callback<photoResponse>() {
            ...
            public void onResponse(Call<photoResponse> call, Response<photoResponse> response) {
                ...
                if (issuccessfull) {
                    ...
                    if(fotoresponse.getEstado().equals(Utils.Status_OK))  {
                        ...
                        dbImage.updatePhoto(photoId,Utils.ESTATUS.SEND.toString(), new Date());

                    }
                    else{
                        message.append("Error from server: ").append(fotoresponse.getExcepcion());

                        logger.error(Utils.appUser.correo,"Error from server: ",message.toString(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                        //sadb: UPDATE PHOTO STATUS
                        dbImage.updatePhoto(photoId,Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                    }
                }
                else {
                    message.append(response.code()).append(" ").append( response.message());

                    logger.error(Utils.appUser.correo,"Error HTTP on upload_Photo: ",message.toString(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                    dbImage.updatePhoto(photoId,Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                }
                // SADB:  Enabled Process
                Upload_Data = false;
            ...
            public void onFailure(Call<photoResponse> call, Throwable t) {
                ...
                logger.error(Utils.appUser.correo,"Failure uploadPhoto.enqueue: ",message.toString(),Utils.LOG_CATEGORY.ASYNCH_DATA.toString(),get_manufacturer(), mContext,AfterAddMinutes());
                dbImage.updatePhoto(photoId,Utils.ESTATUS.ERROR_SEND.toString(),AfterAddMinutes());
                // SADB:  Enabled Process
                Upload_Data = false;
            }
    ...
    private List<oData> getoData () throws ParseException, IOException {
        ...
        Long offset = new Long (60*DELAY_MINUTES);
 ...
 //       List<LoggerContent.LoggerItem> items_logger = dbLog.getlogger(Utils.ESTATUS.NO_SEND);
    ...
    private Date AfterAddMinutes() {
        final long ONE_MINUTE_IN_MILLIS=60000;//millisecs

        Date date = new Date();
        long t= date.getTime();
        Date afterAddingMins=new Date(t + (DELAY_MINUTES * ONE_MINUTE_IN_MILLIS));
